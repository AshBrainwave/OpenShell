...
fn compile_dpu_policy(policy: Option<&ProtoSandboxPolicy>) -> CompiledPolicy {
    let Some(policy) = policy else {
        return CompiledPolicy {
            ...
            warnings: vec!["No sandbox policy returned; DPU proxy policy compiled as deny-all".to_string()],
        };
    };
...
mod tests {
    ...
    fn compile_policy_merges_hosts_and_ports() {
        ...
        policy.network_policies.insert(
            ...
            NetworkPolicyRule {
                name: "api".to_string(),
                endpoints: vec![endpoint("api.openai.com", &[443]), endpoint("api.openai.com", &[8443])],
                binaries: Vec::new(),
            },
        );
